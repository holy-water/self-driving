...
class DrivingClient(DrivingController):
    def __init__(self):
        ...
        # 도로 범위 할당
        self.track_range = int(self.half_road_limit) if int(self.half_road_limit) % 2 != 0 else int(self.half_road_limit) - 1
        # 이전 도로 번호 할당
...
def calculate_steering(self, sensing_info):
    ...
    to_middle = (track_number - (int(self.track_range / 2) + 1)) * self.track_range * 0.5
    # 현재 중앙값에 맞는 핸들값 할당
    middle = -(sensing_info.to_middle + to_middle) / (self.half_road_limit * self.track_range)
    # 현재 도로 각도에 맞는 핸들값 할당
    angle = -(sensing_info.moving_angle - sensing_info.track_forward_angles[0]) / 60
    # 코너에서 핸들값 가중치 할당
...
def get_disable_track_number_list(sensing_info, middle_number):
    ...
    for obstacle in sensing_info.track_forward_obstacles:
        if obstacle['dist'] < sensing_info.speed * 5 / 18 * 1.5:
            for index in range(middle_number - 1):
                if abs(obstacle['to_middle']) < 1 + (3 * index):
...
def calculate_angle_weight(sensing_info, angle):
    weight = 0
    for i in range(2):
        if abs(sensing_info.track_forward_angles[i]) > 18:
            weight = (angle * abs(sensing_info.track_forward_angles[i])) / 18
